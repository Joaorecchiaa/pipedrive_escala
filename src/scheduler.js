...
// Cache de userId por email para reduzir chamadas à API
const userIdCache = {};
...
async function syncEquipes() {
  ...
  console.log(`[Escala] Dia semana: ${dayOfWeek} | Fim de semana: ${isWeekend} | Reset 08h: ${isResetTime}`);
  console.log(`========================================`);

  const escala = await getEscala();
  if (!escala.length) {
    console.log('[Escala] AVISO: Nenhum dado encontrado na planilha.');
    return;
  }
  ...
  for (const [equipeIdStr, membrosEquipe] of Object.entries(porEquipe)) {
    ...
    if (isWeekend) {
      ...
      if (faltando.length > 0) {
        console.log(`[Escala] Fim de semana — adicionando ${faltando.length} pessoa(s)`);
        await addTeamMembers(equipeId, faltando);
      } else {
        console.log(`[Escala] Fim de semana — todos ja estao na equipe`);
      }
      continue;
    }

    // RESET 08h: limpa a equipe e adiciona turno atual
    if (isResetTime) {
      const atuais = await getTeamMembers(equipeId);
      if (atuais.length > 0) {
      ...
      if (turnoAtual.length > 0) {
        ...
        console.log(`[Escala] Reset 08h — adicionando ${ids.length} pessoa(s) do turno`);
        await addTeamMembers(equipeId, ids);
      } else {
        console.log(`[Escala] Ninguem no turno as 08h — fallback: adicionando todos`);
    ...
    if (turnoAtual.length === 0) {
      console.log(`[Escala] Ninguem na escala agora — fallback: adicionando todos`);
    ...
    if (paraRemover.length > 0) {
      console.log(`[Escala] Removendo ${paraRemover.length} pessoa(s) fora do turno`);
      await removeTeamMembers(equipeId, paraRemover);
    }
    if (paraAdicionar.length > 0) {
      console.log(`[Escala] Adicionando ${paraAdicionar.length} pessoa(s) no turno`);
