...
async function syncEquipes() {
  ...
  for (const [equipeIdStr, membrosEquipe] of Object.entries(porEquipe)) {
    ...
    console.log(`[Escala] SDRs na planilha (${membrosEquipe.length}): ${membrosEquipe.map(p => p.nome).join(', ')}`);

    // FIM DE SEMANA
    if (isWeekend) {
      const todosIds = membrosEquipe.map(p => p.userId);
      const atuais   = await getTeamMembers(equipeId);
      console.log(`[Escala] Na equipe agora (${atuais.length} ids): ${JSON.stringify(atuais)}`);
    ...
    if (isResetTime) {
      ...
      console.log(`[Escala] No turno agora (${turnoAtual.length}): ${turnoAtual.map(p => `${p.nome} (${p.entrada}-${p.saida})`).join(', ')}`);
      if (turnoAtual.length > 0) {
        await addTeamMembers(equipeId, turnoAtual.map(p => p.userId));
      } else {
        console.log(`[Escala] Ninguem no turno — fallback: adicionando todos SDRs`);
    ...
    if (turnoAtual.length === 0) {
      console.log(`[Escala] Ninguem na escala agora — todos SDRs entram`);
    ...
    if (paraRemover.length > 0) {
      console.log(`[Escala] Removendo ${paraRemover.length} SDR(s) fora do turno: ${JSON.stringify(paraRemover)}`);
      await removeTeamMembers(equipeId, paraRemover);
    }
    if (paraAdicionar.length > 0) {
      console.log(`[Escala] Adicionando ${paraAdicionar.length} SDR(s) no turno: ${JSON.stringify(paraAdicionar)}`);
