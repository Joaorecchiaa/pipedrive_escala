...
// Cache de userId por email — persiste durante a execução
const userIdCache = {};
...
async function syncEquipes() {
  ...
  for (const [equipeIdStr, membrosEquipe] of Object.entries(porEquipe)) {
    ...
    // FIM DE SEMANA: todos os SDRs entram
    if (isWeekend) {
    ...
    if (isResetTime) {
      ...
      if (turnoAtual.length > 0) {
        ...
        console.log(`[Escala] Reset 08h — adicionando ${ids.length} SDR(s) do turno`);
        await addTeamMembers(equipeId, ids);
      } else {
        console.log(`[Escala] Ninguem no turno as 08h — fallback: adicionando todos SDRs`);
    ...
    if (turnoAtual.length === 0) {
      // Fora do horário comercial ou entre turnos — todos SDRs entram
