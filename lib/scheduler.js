...
function isOnShift(pessoa, currentMinutes, dayOfWeek) {
  if (!isActiveToday(pessoa, dayOfWeek)) return false;
...
function isActiveToday(pessoa, dayOfWeek) {
  // null = todos os dias; array vazio ou undefined = todos os dias
  if (!pessoa.dias || pessoa.dias.length === 0) return true;
...
async function syncEquipes() {
  ...
  // Filtra linhas com equipeId inválido (NaN)
  const escalaValida = escalaComId.filter(p => {
    if (isNaN(p.equipeId)) {
      console.warn(`[Escala] AVISO: equipeId inválido para ${p.nome} — verifique a planilha`);
      return false;
    }
    return true;
  });

  const porEquipe = groupByTeam(escalaValida);
